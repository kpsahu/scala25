...
object SparkCommon {
  ...
  def readFutureXCourseHiveTable(spark: SparkSession): Option[DataFrame] = {
    try {
      println("readFutureXCourseHiveTable method started")
      spark.sql("use fxxcoursedb")
      spark.sql("show tables")
      val courseDF = spark.sql("select * from fx_course_table")
      println("readFutureXCourseHiveTable method ended")
      Some(courseDF)
    } catch {
      case e: Exception =>
        println("Error Reading fxxcoursedb.fx_course_table " + e.printStackTrace())
        None
    }
  }

  def readData(spark: SparkSession): DataFrame={
    val df=spark.read.format("parquet").load("E:\\bigdata\\Scala_projects\\scala_mvn\\spark-warehouse\\fxxcoursedb.db\\fx_course_table")
    df
  }
